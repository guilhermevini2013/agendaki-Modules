...
public record PaymentPixCreateDTO(String reference_id, CustomerDTO customer, List<QRcodeDTO> qr_codes) {
    public PaymentPixCreateDTO(PaymentCreateDTO paymentDTO, PreUser preUser) {
        this(preUser.getId(), new CustomerDTO(preUser.getName(), preUser.getUsername(), paymentDTO.cpf().replace(".", "").replace("-", "")), List.of(new QRcodeDTO(new AmountDTO(paymentDTO.typeSignature().getDateBySignature().price().toString(), "BRL"))));
    }
}
